...
//TO(DONE)DO: add an eventlistener to the button to trigger addMedal
...
function init() {
  ...
  //TO(DONE)DO: populate the Sport select
  let count_sport = 0;
  for (let s in Sports) {
    if (isNaN(Number(s))) {
      let newOption: HTMLOptionElement = document.createElement('option');
  ...
  //TO(DONE)DO: populate the Medal select
  let count_medal = 0;
  for (let m in Medals) {
    if (isNaN(Number(m))) {
      let newOption: HTMLOptionElement = document.createElement('option');
...
function addMedal() {
  //TO(DONE)DO: complete this function
...
function displayTable() {
  ...
  // TO(DONE)DO: create the rows required for the new table body element
  for (let i = 0; i < countries.length; i++) {
